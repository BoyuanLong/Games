...
bool PlayerUI::IsPlayerInFirst() const
{
    ...
    if(pm->GetLap() == em->GetLap()){
        if(pm->GetLastPoint() == em->GetLastPoint())
        {
            Vector3 target = pm->GetNextLocation();
            float pdistance = (target - mOwner->GetPosition()).Length();
            float edistance = (target - enemy->GetPosition()).Length();
            return pdistance < edistance;
        }
        else{
            return pm->GetLastPoint() > em->GetLastPoint();
        }
    }
    else{
        return pm->GetLap() > em->GetLap();
    }
