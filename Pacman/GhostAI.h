...
private:
    ...
    bool PathFinding(PathNode* startNode, PathNode* target);
    ...
    PathNode* FindNodeClosest(const Vector2& loc);
    
    void FailSafe();
    
    void SetDirection();
    
    float heur(PathNode* c, PathNode* t);
    PathNode* findNearest(std::vector<std::pair<PathNode*, int> >& c, PathNode* t);
    
    void ScatterMove();
    
    void Teleport(PathNode* target);
