...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
        ...
        {
            actor = new LaserMine(game, parent);
        }
        else if(type == "Checkpoint")
        {
            ...
            actor = cp;
            std::string str;
            if (GetStringFromJSON(actorValue, "level", str))
            {
                cp->SetString(str);
            }
        }
        ...
        {
            SecurityCamera* camera = new SecurityCamera(game);
